...
async function contractApprovalHandler(req, res) {
    ...
    const documentsArray = documents.map(document => document.get({ plain: true }));

    // const { assignee_position, assignee_department } = documentsArray[0];

    const { approver_position, approver_department } = documentsArray[0];

    const approversEmails = await Promise.all(documentsArray.map(async({ assignee_position, assignee_department }) => {
    ...
    // const approvers = await User.findOne({
    //     where: {
    //         position: assignee_position,
    //         department: assignee_department,
    //     },
    ...
    // const approversArray = approvers.map(approver => approver.get({ plain: true }));

    console.log('approversEmailsapproversEmailsapproversEmailsapproversEmailsapproversEmails: ', approversEmails);
